...
class Input
{
     ...
     * @param string $custom_regex Used when $type is set to "custom". Does not
     *  use $min_length, any length requirement must be included in the regex
     ...
     *
     * @param array $rules A multi-deminsional array, where the 1st dimension
     *  is the index value of the data given to Input::validates()
     * @see Input::validates()
     */
     ...
     * @param array $var An array containing the full string index of the rule
     *  to evaluate ('index') and the complete rule ('rule')
     ...
     * - post_format
     *
     * pre_format and post_format accept a typical PHP callback parameter.
     * post_format will only be called if the rule passes validation.
     *
    ...
    private function validateRule($index, $rule, &$value, $key, $path = array())
    {
        ...
        foreach ($rule as $type => $rule_set) {
            ...
            if ((isset($rule_set['negate']) && $rule_set['negate'] && !$response)
                || ((!isset($rule_set['negate']) || !$rule_set['negate']) && $response)
            ) {
     ...
     * @param array $path A list of all array indexes encountered
     * @return boolean False if the input is no longer an array and therefore
     *  can not be recursed through, true otherwise
     */
    private static function arrayWalkRecursive(
        &$input,
        $callback,
        $params = null,
        $max_depth = null,
     ...
     * @param mixed $field An array defining a path that $data should define,
     *  scalar if the array has been fully traversed
     ...
     *
     * @param mixed $data An array of data whose leaves to set to null, or a
     *  scalar value if the array has been fully traversed
     */
